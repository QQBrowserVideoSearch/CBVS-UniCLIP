...
from uniclip.clip import load
from uniclip.clip.model import convert_weights, convert_state_dict, resize_pos_embed, CLIP
#from uniclip.training.train import train, evaluate
from uniclip.training.train import train, evaluate
from uniclip.training.params import parse_args
from uniclip.training.logger import setup_primary_logging, setup_worker_logging
from uniclip.training.scheduler import cosine_lr
...
def main():
    ...
    logging.info("find_unused_parameters:{}".format(find_unused_parameters))
    ...
    if args.resume is not None:
        if os.path.isfile(args.resume):
            ...
            # Load the state dict
            model.load_state_dict(sd, strict=False) # 在此处加载的模型
