...
from uniclip.clip.configuration_bert import BertConfig
from uniclip.clip.modeling_bert import BertModel, BertEmbeddings
...
class ResidualAttentionBlockDecoder(nn.Module):
    ...
    def forward(self, tgt, memory):
        tgt = tgt + self.self_attn(self.ln_1(tgt), self.ln_1(tgt), self.ln_1(tgt), need_weights=False)[0]
        tgt = tgt + self.cross_attn(self.ln_2(tgt), self.ln_2(memory), self.ln_3(memory), need_weights=False)[0]
        tgt = tgt + self.mlp(self.ln_4(tgt))
        return tgt
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.ocr_exist=ocr_exist
        self.ocr_content=ocr_content
        
        if self.ocr_exist:
            self.ocr_exist_decoder = Transformer(vision_width, layers=3, heads=vision_heads, use_flash_attention=use_flash_attention)
            self.ln_ocr_exist = LayerNorm(vision_width)
            self.proj_ocr_exist = nn.Parameter(torch.randn(vision_width, 2))
        ...
        if self.ocr_content:
            ...
            self.ocr_content_decoder = TransformerDecoder(vision_width, layers=3, heads=vision_heads, use_flash_attention=use_flash_attention)
            self.ln_ocr_content = LayerNorm(vision_width)
            self.proj_ocr_content = nn.Parameter(torch.randn(vision_width, 2))
    ...
    def forward(self, image, text, fake_ocrs=None, mask_ratio=0):
        ...
        # from raw featrues, to predict whether ocr exists or not
        if self.ocr_exist and not self.ocr_content:
            raw_features = raw_features.permute(1, 0, 2)  # NLD -> LND
            raw_features = self.ocr_exist_decoder(raw_features)
            raw_features = raw_features.permute(1, 0, 2)  # LND -> NLD
            raw_features = self.ln_ocr_exist(raw_features[:, 0, :])
            ocr_exist_est = raw_features @ self.proj_ocr_exist

            output = {'image_features': image_features,
                    'text_features': text_features,
                    'logit_scale': self.logit_scale.exp(),
                    'ocr_exist_est': ocr_exist_est}
            # ------------------------------------------------------------------------
            return output
        elif self.ocr_exist and self.ocr_content:
            raw_features = raw_features.permute(1, 0, 2)  # NLD -> LND
            ...
            ocr_exist_features = self.ocr_exist_decoder(raw_features)
            ocr_content_features = self.ocr_content_decoder(fake_ocrs, raw_features)
            ocr_exist_features = ocr_exist_features.permute(1, 0, 2)  # LND -> NLD
            ocr_content_features = ocr_content_features.permute(1, 0, 2)  # LND -> NLD
            ocr_exist_features = self.ln_ocr_exist(ocr_exist_features[:, 0, :])
            ocr_exist_est = ocr_exist_features @ self.proj_ocr_exist
            ocr_content_features = self.ln_ocr_content(ocr_content_features[:, 0, :])
            ocr_content_est = ocr_content_features @ self.proj_ocr_content

            output = {'image_features': image_features,
                    'text_features': text_features,
                    'logit_scale': self.logit_scale.exp(),
                    'ocr_exist_est': ocr_exist_est,
                    'ocr_content_est': ocr_content_est
                    }
            return output
        elif not self.ocr_exist and self.ocr_content:
            raw_features = raw_features.permute(1, 0, 2)  # NLD -> LND
            fake_ocrs = self.bert_embedding(fake_ocrs)
            fake_ocrs = torch.cat([self.decoder_class_embedding.to(fake_ocrs.dtype) + torch.zeros(fake_ocrs.shape[0], 1, fake_ocrs.shape[-1], dtype=fake_ocrs.dtype, device=fake_ocrs.device), fake_ocrs], dim=1)  # shape = [*, grid ** 2 + 1, width]
            fake_ocrs = fake_ocrs.permute(1, 0, 2)
