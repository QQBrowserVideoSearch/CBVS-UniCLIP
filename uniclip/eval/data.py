...
class EvalDataset(Dataset):
    def __init__(self):
        self.dic = {'0/缺失':0, '1/弱相关':1, '2/相关':2, '-1/不相关':-1}
        self.text_ids = []
        self.querys = []
        self.imgs = []
        self.preprocess = image_transform(224)

        with open('/search/odin/1_ceceliali/12_cover_paper/2_exp_models/2w_biaozhu_cover_final_label_v2.txt', 'r') as fin:
            for line in tqdm(fin):
                ...
                self.imgs.append(f'/search/odin/xsqiao/UniOCR/Code/pnr_utils/imgs/{docid}.jpg')
...
def get_eval_ocr_dataset(args, max_txt_length=20):
    input_filename = args.text_data
    dataset = EvalOCRDataset(
        input_filename,
        max_txt_length=max_txt_length)
    num_samples = len(dataset)
    sampler = SequentialSampler(dataset)
